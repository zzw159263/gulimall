    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/smsskuladder/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.smsSkuLadder.skuId
                this.dataForm.fullCount = data.smsSkuLadder.fullCount
                this.dataForm.discount = data.smsSkuLadder.discount
                this.dataForm.price = data.smsSkuLadder.price
                this.dataForm.addOther = data.smsSkuLadder.addOther
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/smsskuladder/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
