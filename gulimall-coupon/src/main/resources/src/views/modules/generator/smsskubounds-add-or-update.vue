    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/smsskubounds/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.smsSkuBounds.skuId
                this.dataForm.growBounds = data.smsSkuBounds.growBounds
                this.dataForm.buyBounds = data.smsSkuBounds.buyBounds
                this.dataForm.work = data.smsSkuBounds.work
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/smsskubounds/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
