    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/smscategorybounds/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.categoryId = data.smsCategoryBounds.categoryId
                this.dataForm.growBounds = data.smsCategoryBounds.growBounds
                this.dataForm.buyBounds = data.smsCategoryBounds.buyBounds
                this.dataForm.work = data.smsCategoryBounds.work
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/smscategorybounds/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
